...
public class AiIntegrationServiceImpl implements AiIntegrationService {
    ...
    public Mono<String> forwardRequest(String path, Object requestBody, String jwtToken) {
        ...
        // System.out.println("Forwarding request to: " + path + " with token: " +
        // jwtToken);
        ...
        return this.webClient.post()
                ...
                .switchIfEmpty(Mono.just(
                        "{\"output\": \"AI servisinden boş bir yanıt alındı. Lütfen tekrar deneyin.\", \"suggestions\": []}"))

                // 2. Eğer bir hata olursa (timeout, 500 hatası vb.), bunu logla
                // ve yine geçerli bir hata JSON'u döndür.
                .doOnError(error -> logger.error("FastAPI isteği başarısız oldu. Path: {}, Hata: {}", path,
                        error.getMessage()))
                .onErrorResume(error -> Mono.just(
                        "{\"output\": \"AI servisi şu anda mevcut değil veya bir hata oluştu. Lütfen daha sonra tekrar deneyin.\", \"suggestions\": []}"));
    }
}
